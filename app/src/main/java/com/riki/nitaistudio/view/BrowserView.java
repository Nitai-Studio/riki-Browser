package com.riki.nitaistudio.view;
...
import com.riki.nitaistudio.downloads.DownloadHandler;
import com.riki.nitaistudio.downloads.DownloadStart;
import com.riki.nitaistudio.utils.FetchTitle;
...
import com.riki.nitaistudio.view.webClient.ChromeClient;
...
public class BrowserView {
    ...
    @SuppressLint({"ClickableViewAccessibility"})
    public BrowserView(@NonNull Activity activity, @Nullable String str, boolean incognito) {
        ...
        if (this.isIncognito) {
            // Note: Android's CookieManager is process-wide, not per-WebView,
            // so cookies cannot be isolated to just this tab without a
            // separate WebView data directory (a much bigger change). What we
            // CAN guarantee per-tab: no disk cache, no DOM storage/IndexedDB,
            // no WebSQL, no saved form data, and this tab is skipped entirely
            // by saveState() so it never survives an app restart.
            WebSettings incognitoSettings = this.mWebView.getSettings();
            incognitoSettings.setCacheMode(WebSettings.LOAD_NO_CACHE);
            incognitoSettings.setDomStorageEnabled(false);
            incognitoSettings.setDatabaseEnabled(false);
            incognitoSettings.setSaveFormData(false);
        }
