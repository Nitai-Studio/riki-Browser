package com.riki.nitaistudio.view.webClient;
...
public class WebClient extends WebViewClient {
    ...
    public WebResourceResponse shouldInterceptRequest(WebView webView, @NonNull WebResourceRequest request) {
        // Never block the main document request itself - only sub-resources
        // (ad scripts, tracker pixels, etc). Blocking the main frame is what
        // was causing entire pages/sites to come up blank when their domain
        // happened to match an entry in hosts.txt.
    ...
    public WebResourceResponse shouldInterceptRequest(WebView webView, String url) {
        // Legacy pre-API21 callback: only called on devices below minSdk (21),
        // kept only for safety. Same main-frame guard as above.
        if (webView != null && url != null && url.equals(webView.getUrl())) {
            return null;
        }
        if (mAdBlock.isAd(url)) {
            return new WebResourceResponse("text/plain", "utf-8", new ByteArrayInputStream("".getBytes()));
